...
public class CronExpression implements Serializable, Cloneable {
	 ...
	 * @param date
	 * @param calcMode
	 *            --0按照cron表达式计算；1按照简单每隔多少秒计算,...待扩展
	 * @return
	 */
	public Date getNextValidTimeAfter(Date date, int calcMode) {
		if (calcMode == 0) {
			return getTimeAfter(date);
		} else if (calcMode == 1) {
			int i = this.getSimpleIntervalSecond();
			if (i == 0) {// 按照分钟算
				i = this.getSimpleIntervalMinute();
				long x = i * 60 * 1000;
				long y = date.getTime() + x;
				return new Date(y);
			} else {// 按照秒算
				long x = i * 1000;
				long y = date.getTime() + x;
	...
	private static boolean isSimpleIntervalModeExpression_tmp(String xEps) {
		boolean f = false;
		String a[] = xEps.split(" ");
		if (a[2].equals("*") && a[3].equals("*") && a[4].equals("*") && a[5].equals("?")) {
			if (a[0].indexOf('/') > 0 || a[1].indexOf('/') > 0) {
				f = true;
			}
		}
		OtherUtil.clearArray(a);
		return f;
	}

	/**
	 * 判别是否为简单的间隔的表达式: 每隔多少秒或每隔多少分钟
	 * 
	 * @return
	 */
	public boolean isSimpleIntervalModeExpression() {
		return isSimpleIntervalModeExpression_tmp(this.cronExpression);
	}

	private static String formateStr(String b) {
		b = b.substring(2, 4);
		int i = b.lastIndexOf(',');
		if (i > 0) {
			b = b.substring(0, i);
	 ...
	 * @return
	 */
	public int getSimpleIntervalSecond() {
		String a = getExpressionSetSummary(seconds);
		if (a.length() <= 1) {
			return 0;
		}
		a = formateStr(a);
	 ...
	 * @return
	 */
	public int getSimpleIntervalMinute() {
		String a = getExpressionSetSummary(minutes);
		if (a.length() <= 1) {
			return 0;
		}
		a = formateStr(a);
