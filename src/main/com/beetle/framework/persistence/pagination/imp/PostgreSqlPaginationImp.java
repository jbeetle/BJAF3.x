 ...
 * @version 1.0
 */
public class PostgreSqlPaginationImp implements IPagination {
	private static ICache countCache = new TimeOutCache(120);
	private static AppLogger logger = AppLogger
			.getInstance(PostgreSqlPaginationImp.class);

	public PostgreSqlPaginationImp() {
	...
	public PageResult page(PageParameter pInfo) throws PaginationException {
		...
		try {
			...
			if (pos >= 0) {
				...
				// sb.append(" limit " + bn + " offset " + pInfo.getPageSize());
				sb.append(" limit " + pInfo.getPageSize() + " offset " + bn);
