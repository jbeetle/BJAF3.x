...
public abstract class WebServiceController extends AbnormalViewControlerImp {
	private static final AppLogger logger = AppLogger.getInstance(WebServiceController.class);
	...
	private static class JacksonCreator {
		...
		public void toJSONJackson(OutputStream outputStream, final ModelData md, String ecode)
				throws ControllerException {
			org.codehaus.jackson.JsonGenerator jsonGenerator = null;
			try {
				jsonGenerator = objectMapper.getJsonFactory().createJsonGenerator(outputStream, getEncoding(ecode));
		...
		private org.codehaus.jackson.JsonEncoding getEncoding(String ecode) {
			for (org.codehaus.jackson.JsonEncoding encoding : org.codehaus.jackson.JsonEncoding.values()) {
	...
	private static class XstreamCreator {
		...
		public void toJSONXtream(OutputStream outputStream, final ModelData md, String ecode) {
		...
		public void toXml(OutputStream outputStream, final ModelData md, String ecode) {
			com.thoughtworks.xstream.XStream xtm = new com.thoughtworks.xstream.XStream();
			final String dd;
			if (md.isDataBeanFormatReturn()) {
				dd = xtm.toXML(md.getData());
	...
	@Override
	public void performX(WebInput webInput, OutputStream outputStream) throws ControllerException {
		...
		if (wso != null || actionName == null || actionName.length() == 0) {
			...
			MethodEx method = ControllerHelper.getActionMethod(webInput.getControllerName(), actionName, this,
					WebInput.class);
			try {
		...
		if (md == null || outputStream == null) {
			throw new ControllerException("service return is null,service must have data to return!");
		}
		...
		if (md.getDataType().equals(ModelData.DataType.XML)) {
			...
			if (jflag.equalsIgnoreCase("XStream")) {
				XstreamCreator.getInstance().toJSONXtream(outputStream, md, ecode);
			} else {
	...
	private void dealOtherCase(WebInput webInput, OutputStream outputStream, final ModelData md, String ecode)
			throws ControllerException {
		...
		if (selfDef.equalsIgnoreCase("xml")) {
			...
			String defaultFormat = (String) webInput.getRequest()
					.getAttribute(CommonUtil.WEB_SERVICE_DATA_DEFAULT_FORMAT);
			if (defaultFormat.equalsIgnoreCase("xml")) {
				...
				throw new ControllerException("not support [" + defaultFormat + "] format yet!");
			}
		} else {
			throw new ControllerException("not support [" + selfDef + "] format yet!");
		}
	}
	 ...
	 * @throws ControllerException
	 */
	public abstract ModelData defaultAction(WebInput webInput) throws ControllerException;
}
