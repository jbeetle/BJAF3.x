/*
 * BJAF - Beetle J2EE Application Framework
 * 甲壳虫J2EE企业应用开发框架
 * 版权所有2003-2015 余浩东 (www.beetlesoft.net)
 * 
 * 这是一个免费开源的软件，您必须在
 *<http://www.apache.org/licenses/LICENSE-2.0>
 *协议下合法使用、修改或重新发布。
 *
 * 感谢您使用、推广本框架，若有建议或问题，欢迎您和我联系。
 * 邮件： <yuhaodong@gmail.com/>.
 */
package com.beetle.framework.web.controller.document;

import com.lowagie.text.Document;
import org.apache.poi.hssf.usermodel.HSSFWorkbook;

/**
 * <p>
 * Title: BeetleWeb
 * </p>
 * 
 * <p>
 * Description: MVC Web Framework
 * </p>
 * 
 * <p>
 * Copyright: Copyright (c) 2005
 * </p>
 * 
 * <p>
 * Company: 甲壳虫软件
 * </p>
 * 
 * @author 余浩东(hdyu@beetlesoft.net)
 * @version 1.0
 */
public class DocInfo {
	private int docType;
	private Document pdfDocument;
	private HSSFWorkbook excelDocument;

	public DocInfo(int docType) {
		this.docType = docType;
	}

	/**
	 * 获取一个PDF文档对象 com.lowagie.text.Document
	 * 
	 * @return Document
	 */
	public Document getPdfDocument() {
		return pdfDocument;
	}

	public int getDocType() {
		return docType;
	}

	/**
	 * 获取一个Excel文档对象 org.apache.poi.hssf.usermodel.HSSFWorkbook
	 * 
	 * @return HSSFWorkbook
	 */
	public HSSFWorkbook getExcelDocument() {
		return excelDocument;
	}

	public void setPdfDocument(Document pdfDocument) {
		this.pdfDocument = pdfDocument;
	}

	public void setExcelDocument(HSSFWorkbook excelDocument) {
		this.excelDocument = excelDocument;
	}
}
